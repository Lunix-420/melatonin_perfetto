...
PERFETTO_DEFINE_CATEGORIES(
  perfetto::Category("component").SetDescription("Component"),
  perfetto::Category("dsp").SetDescription("dsp"));
...
public:
  ...
  // Returns the file where the dump was written to (or a null file if an error
  // occurred) the return value can be ignored if you don't need this
  // information
...
private:
  ...
  juce::File writeFile()
  {
    ...
    const auto currentTime =
      juce::Time::getCurrentTime().formatted("%Y-%m-%d_%H%M");
    const auto childFile =
      file.getChildFile("perfetto" + mode + currentTime + ".pftrace");
...
/*
   ...
   Deriving the function name produces an ugly string:
        auto AudioProcessor::processBlock(juce::AudioBuffer<float> &,
   juce::MidiBuffer &)::(anonymous class)::operator()()::(anonymous
   class)::operator()(uint32_t) const

   Without dragons, trying to trim the string results in it happening at
   runtime.
...
namespace melatonin {
// This wraps our compile time strings (coming from PRETTY_FUNCTION, etc) in
// lambdas so that they can be used as template parameters This lets us trim the
// strings while still using them at compile time
...
constexpr auto
compileTimePrettierFunction(CompileTimeLambdaWrappedString wrappedSrc)
{
...
constexpr bool
strings_equal(const std::array<char, sizeResult>& result,
              const char (&test)[sizeTest])
{
  // sanity check
...
// testing at compile time isn't fun (no debugging) so dumb things like this
// help:
constexpr std::array<char, 10> main{ "main" };
static_assert(strings_equal(main, "main"));

// ensure the return type is removed
static_assert(strings_equal(
  compileTimePrettierFunction(WRAP_COMPILE_TIME_STRING("int main")),
  "main"));

// clang example
static_assert(strings_equal(
  compileTimePrettierFunction(WRAP_COMPILE_TIME_STRING(
...
// The separate constexpr calls are required for `compileTimePrettierFunction`
// to remain constexpr in other words, they can't be inline with
// perfetto::StaticString, otherwise it will go runtime

// we also can toggle dsp/component on/off individually to help clean up traces
#if PERFETTO_ENABLE_TRACE_DSP
#define TRACE_DSP(...)                                                         \
  constexpr auto pf = melatonin::compileTimePrettierFunction(                  \
    WRAP_COMPILE_TIME_STRING(PERFETTO_DEBUG_FUNCTION_IDENTIFIER()));           \
